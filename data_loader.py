...
def bulid_vocab_id():
    ...
    f = open("./data/cnews.vocab.txt", "r", encoding="utf-8")
    vocab = f.readlines()
    f.close()
...
def build_category_id(filepath):
    categories=[]
...
# 将文本转换为id表示
def word_to_id(file_path, vocab_size, max_length,args):
    vocab, vocab_id = bulid_vocab_id()
    train_dir = os.path.join('.', args.train_data + "/cnews.train.txt")
    cat_to_id = build_category_id(train_dir)
